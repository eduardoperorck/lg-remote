/* A tela do controle. Sem framework: é uma grade de botões e uma lista de
   resultados — dependência de UI aqui só atrapalharia.

   Este arquivo veio inteiro do PWA. A única mudança foi o transporte: onde havia
   `fetch` para um servidor no PC, agora há uma chamada direta ao roteador que roda
   dentro do próprio telefone. O resto — abas, direcional, gravador, busca — não
   soube da diferença. */
...
/* Mesma assinatura de antes; o que mudou é quem atende. Sem rede, sem PIN, sem
   servidor: o roteador é uma função no mesmo processo. */
const api = (path, options = {}) => window.lgremote.api(path, options);
...
/* Isto não existia no PWA: quando a TV recusava a chave, a única saída era voltar
   ao PC e rodar `lgremote pair`. Era uma das três formas de "perder o pareamento". */

function gateMessage(text) {
  const status = el("gate-status");
...
async function pairWith(host, port) {
  gateError("");
  gateMessage("Olhe a TV: ela vai pedir sua autorização. Aceite com o controle físico.");
  try {
    await post("/api/pair", { host, port });
    gateMessage("");
    gate.hidden = true;
    await enter();
  } catch (err) {
...
el("gate-scan").addEventListener("click", async () => {
  const list = el("gate-list");
  gateError("");
  gateMessage("Procurando a TV na rede…");
  list.hidden = true;
  list.innerHTML = "";
  try {
    const { candidates } = await post("/api/pair/scan");
    if (!candidates.length) {
      gateMessage("");
      gateError("Não achei nenhuma TV. Ela está ligada e no mesmo Wi-Fi?");
      return;
    }
    gateMessage("");
    for (const candidate of candidates) {
      ...
      button.textContent = `${candidate.host}:${candidate.port}`;
      button.addEventListener("click", () => pairWith(candidate.host, candidate.port));
...
el("gate-form").addEventListener("submit", (event) => {
  ...
  /* Aceita "192.168.2.107" e "192.168.2.107:3000": a porta explícita serve às TVs
     de 2014-2017, que só falam na 3000, e ao servidor de mentira do modo dev. */
  const [host, port] = el("gate-host").value.trim().split(":");
  if (host) pairWith(host, Number(port) || 3001);
});
...
if (window.lgremote.paired) {
  enter().catch((err) => {
    // Chave recusada pela TV cai aqui: em vez de uma tela morta, o pareamento
    // fica a um toque de distância.
    showGate();
    gateError(err.message);
  });
} else {
